...
def evaluate_imagenet_sketch(model, data_dir, test_batchsize=128, test_transform=None, amp_autocast=None, dist=False):
    ...
    if test_transform is None:
        insk_transform = transforms.Compose([transforms.Resize(256, interpolation=InterpolationMode.BICUBIC), # 256
    ...
    top1_m = AverageMeter()
    for input, target in (insk_data_loader):
        input = input.cuda()
        target = target.cuda()
        with torch.no_grad():
            if amp_autocast:
