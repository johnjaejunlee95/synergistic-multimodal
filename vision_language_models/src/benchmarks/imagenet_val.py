...
def evaluate_imagenet_val(model, data_dir, test_batchsize=128, test_transform=None, amp_autocast=None, dist=False):
    ...
    if test_transform is None:
        inv2_transform = transforms.Compose([transforms.Resize(256, interpolation=InterpolationMode.BICUBIC), # 256
    ...
    top1_m = AverageMeter()
    # model.eval()
    for input, target in (inv2_data_loader):
        input = input.cuda()
        target = target.cuda()
        with torch.no_grad():
            if amp_autocast:
