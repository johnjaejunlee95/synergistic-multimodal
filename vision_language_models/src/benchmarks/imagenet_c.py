...
def evaluate_imagenet_c(model, data_dir, test_batchsize=128, test_transform=None, amp_autocast=None, dist=False):
    ...
    if test_transform is None:
        # inc_transform = transforms.Compose([transforms.Resize(256, interpolation=InterpolationMode.BICUBIC), # 256
    ...
    for name, subdir in data_loaders_names.items():
        ...
        for severity in range(1, 6):
            ...
            top1_m = AverageMeter()
            # model.eval()
            for input, target in (inc_data_loader):
                input = input.cuda()
                target = target.cuda()
                with torch.no_grad():
                    if amp_autocast:
